...
class Game_Board():
    ...
    def __init__(self):
        ...
        self.white_mill_dict = {}            # sve mill_dictove napravi kao set, u ovom slucaju ti bolje !!!
    ...
    def update_gui(self):
        ...
        while self.running:
            ...
            if self.PLAYER_ON_MOVE == WHITE and mouse_click:
                ...
                print('Crne figure prije mlina: {}'.format(self.black_positions_taken.keys()))
                ...
                if self.NUMBER_OF_PIECES: print('PRVA FAZA CRNI:')
                elif not self.NUMBER_OF_PIECES and self.BLACK_PIECES > 3: print('DRUGA FAZA CRNI. upisi prvo poziciju koju zelis pomaknuti onda kamo pomaknuti')
                ...
                if choose_place in position_on_board:
                    ...
                    if is_legal_move_made:
                        print('Trenutno u crnom mlinu: {}'.format(self.black_mill_dict.keys()))
                        if self.is_mill(BLACK):
                            self.make_mill_move_black = True
                            self.PLAYER_ON_MOVE = 0
                        else: self.PLAYER_ON_MOVE = WHITE
                    print('Sljedeci na redu: {}'.format(self.PLAYER_ON_MOVE))
                    print('Trenutni u crnom mlinu: {}'.format(self.black_mill_dict.keys()))
            ...
            if self.white_positions_taken and self.no_legal_moves_game_loss(WHITE) or self.WHITE_PIECES < 3:
                print('Crni je pobjedio.')
                pygame.quit()
            if self.black_positions_taken and self.no_legal_moves_game_loss(BLACK) or self.BLACK_PIECES < 3:
    ...
    def is_mill(self, player_color):
        ...
        if player_color == self.white:
            wpt =  copy.deepcopy(self.white_positions_taken)
            #wmd = self.white_mill_dict
            wmh = copy.deepcopy(self.white_mill_dict_helper)
        else:
            wpt = copy.deepcopy(self.black_positions_taken)
            #wmd = self.black_mill_dict
            wmh = copy.deepcopy(self.black_mill_dict_helper)
        ...
        if player_color == self.white:
            self.white_mill_dict = copy.deepcopy(wmd)
            self.white_mill_dict_helper = copy.deepcopy(wmh)
        else:
            self.black_mill_dict = copy.deepcopy(wmd)
            self.black_mill_dict_helper = copy.deepcopy(wmh)
