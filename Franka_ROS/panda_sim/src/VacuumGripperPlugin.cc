...
void VacuumGripperPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  {
    gzdbg << "No grippable model types" << "\n";
  }
...
void VacuumGripperPlugin::HandleAttach()
{
  ...
  {
    gzdbg << "Product already attached!" << std::endl;
    return;
  }
...
bool VacuumGripperPlugin::CheckModelContact()
{
      ...
      !this->dataPtr->attached)
  {
    ...
    if (alignment > 0.95) {
      ...
      gzdbg << "Model not aligned. Alignment = " << alignment << std::endl;
      gzdbg << "Gripper link normal: " << gripperLinkNormal << std::endl;
      gzdbg << "Model contact normal: " << this->dataPtr->modelContactNormal << std::endl;
    }
  }
